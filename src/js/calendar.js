...
export function loadCalendarData() {
    fetch('events.json')
      ...
      .then(activities => {
        ...
        for (const [date, dayInfo] of Object.entries(activities)) {
          const dayContainer = document.createElement('div');
          dayContainer.classList.add('col-12', 'mb-4');
  
          const dayHeading = document.createElement('h3');
          dayHeading.textContent = `${dayInfo.day} - ${date}`;
      ...
      .catch(error => console.error('Error fetching the events data:', error));
  }
